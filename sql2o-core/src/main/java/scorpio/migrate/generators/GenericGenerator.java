...
public class GenericGenerator implements Generator {
	...
	protected Column getExistingColumn(String columnName, String tableName) {
				...
				if (resultSet != null) {
					while (resultSet.next()) {
						...
						if (col != null && col.equalsIgnoreCase(columnName)) {
							...
							accuracy = resultSet.getInt("DECIMAL_DIGITS");//精度
							defaultObject = resultSet.getObject("COLUMN_DEF");
							if(accuracy != 0){
								columnDefinition = GeneratorHelper.getSqlName(type) + "(" + length + "," + accuracy + ")";
							}
		...
		if (type >= 0) {
			return new Column(columnName, type, length, isPrimaryKey, nullable, defaultObject,columnDefinition, isAutoIncrement,describe);
		} 
