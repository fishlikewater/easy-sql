...
public class MySQLGenerator extends GenericGenerator {
    ...
    protected String makeColumnString(Column column, boolean suppressPrimaryKey) {
		...
		if(StringUtils.isNotBlank(columnDefinition)){
			retVal.append(columnDefinition);
		}else{
			retVal.append(GeneratorHelper.getSqlName(type));
		}
    	if (GeneratorHelper.needsLength(type) && StringUtils.isBlank(columnDefinition)) {
    	    retVal.append("(")
    	...
    	if(StringUtils.isNotBlank(column.getDescribe())){
            retVal.append(" COMMENT '").append(column.getDescribe()).append("' ");
        }
	...
	public String renameColumn(String newColumnName, String oldColumnName,
		...
		Column newColumn = new Column(newColumnName, column.getColumnType(), column.getLength(), column.isPrimaryKey(), column.isNullable(), column.getDefaultValue(),column.getColumnDefinition(), column.isAutoincrement(),column.getDescribe());
